...
/**
 * POST /api/{sessionId}/tickets/[ticketId]/finished
 * Mark a ticket as completed - transitions from 'done' to 'completed' status (session-scoped)
 *
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    logger.info(
      ...
      '[debug] About to trigger waiting tickets from /finished route (session-scoped)',
      {
        ticketId,
        workspaceId,
      }
    ...
    logger.info(
      { category: logCategories.API_TICKETS },
      '[debug] triggerWaitingTickets completed in /finished route (session-scoped)',
      {
        ticketId,
        workspaceId,
      }
    ...
    logger.info(
      { category: logCategories.API_TICKETS },
      'Ticket completed via /finished (session-scoped)',
      {
        ticketId,
