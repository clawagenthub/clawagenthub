...
/**
 * PUT /api/{sessionId}/statuses/[id]
 * Update an existing status (session-scoped, admin/owner only)
 */
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const member = db
      ...
      .get(workspaceId, verification.userId) as
      | { role: string }
      | undefined
    ...
    logger.error('Error updating status (session-scoped):', error)
    return NextResponse.json(
...
/**
 * DELETE /api/{sessionId}/statuses/[id]
 * Delete a status (session-scoped, admin/owner only)
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (ticketsUsingStatus.length > 0) {
      ...
      const now = new Date().toISOString()
      db.prepare('UPDATE tickets SET status_id = ?, updated_at = ? WHERE status_id = ?')
        .run(replacementStatus.id, now, id)
    ...
    logger.error('Error deleting status (session-scoped):', error)
    return NextResponse.json(
      { message: 'Internal server error' },
      { status: 500 }
    )
