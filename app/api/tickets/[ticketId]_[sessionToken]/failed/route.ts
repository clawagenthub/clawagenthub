...
/**
 * POST /api/tickets/[ticketId]_[sessionToken]/failed
 * Agent callback to mark flow step as failed (session token in URL path)
 */
export async function POST(request: NextRequest, context: RouteParams) {
  const { ticketId_sessionToken } = await context.params
  const [ticketId, sessionToken] = ticketId_sessionToken.split('_', 2)
  return processFlowPost(request, { params: Promise.resolve({ ticketId }) }, 'failed', sessionToken)
}