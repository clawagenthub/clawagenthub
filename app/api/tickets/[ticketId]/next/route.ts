...
async function getFlowConfigs(db: any, ticket: Ticket): Promise<FlowConfigs | { error: NextResponse }> {
  ...
  if (currentStatus.name.toLowerCase() === 'done') return { error: NextResponse.json({ error: 'INVALID_TRANSITION', message: "Cannot advance from 'done' - use /finished", hint: 'Call POST /finished' }, { status: 400 }) }
  ...
  if (!nextFlowConfig) return { error: NextResponse.json({ error: 'INVALID_TRANSITION', message: 'No next stage exists in the flow', hint: 'Use /finished' }, { status: 400 }) }
