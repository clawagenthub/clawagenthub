 ...
 *
 * A copy has been preserved in components/deprecated/pairing-modal.tsx
 */
...
import { CommandBlock } from './pairing-modal-parts'
...
type PairingStatus = 'not_started' | 'pending' | 'approved' | 'rejected'
type ConnectionMethod = 'pairing' | 'token'

function GatewayInfo({ name, url }: { name: string; url: string }) {
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
      <h3 className="font-medium text-blue-900 mb-2">{name}</h3>
...
function PendingMessage({ deviceId }: { deviceId: string }) {
  return (
    <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {deviceId && (
            <p className="text-sm mb-2">Device ID: <code className="bg-yellow-100 px-1 rounded font-mono text-xs">{deviceId}</code></p>
          )}
          <p className="text-sm">
            Waiting for approval on the gateway. Please approve this device in the OpenClaw Control UI or via CLI.
          </p>
          <p className="text-xs mt-2 text-yellow-700">
            This page will automatically update when the pairing is approved.
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-4">
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-start">
        ...
        <div className="text-sm text-yellow-800">
          <strong>Note:</strong> Pairing requests expire after 5 minutes
        </div>
      </div>
      ...
      <div className="space-y-4 text-sm">
        <div>
          <p className="text-gray-700 mb-2">
            <span className="font-semibold">1.</span> Open your OpenClaw Control UI (web dashboard)
          </p>
        ...
        <div>
          <p className="text-gray-700 mb-2">
            <span className="font-semibold">2.</span> In the Control UI, navigate to the Devices/Nodes section
          </p>
          <div className="ml-4 text-xs text-gray-600">
        ...
        <div>
          <p className="text-gray-700 mb-2">
            <span className="font-semibold">3.</span> Click "Approve" on the pending device request
          </p>
        </div>

        <div>
          <p className="text-gray-700 mb-2">
            <span className="font-semibold">4.</span> Click "I Paired" button below to verify the connection
          </p>
        </div>

        <details className="mt-3 bg-gray-100 rounded p-3">
          <summary className="cursor-pointer text-sm font-medium text-gray-700 hover:text-gray-900">
            Alternative: Approve via Terminal (CLI)
          </summary>
      ...
      <details className="mt-4">
        ...
        <TroubleshootingSection copiedCommand={copiedCommand} onCopy={onCopy} />
      </details>
    </div>
  )
}
...
function TokenConnectionForm({
  ...
  return (
    <>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-4">
        <div>
          <label htmlFor="gateway-token" className="block text-sm font-medium text-gray-700 mb-2">
            Gateway Token
          </label>
          <input
            id="gateway-token"
          ...
          <p className="mt-2 text-sm text-gray-600">
            The gateway token can be found in your OpenClaw configuration file
            (<code className="bg-gray-200 px-1 rounded text-xs">openclaw.json</code>) under
            <code className="bg-gray-200 px-1 rounded text-xs">gateway.auth.token</code>
          </p>
        </div>
      ...
      <div className="flex justify-end gap-3 pt-4">
        <Button type="button" onClick={onCancel} disabled={checking} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
          Cancel
        </Button>
        <Button type="button" onClick={onSubmit} disabled={checking || !token.trim()} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50">
...
export function PairingModal({
  ...
  const [pairingStatus, setPairingStatus] = useState<PairingStatus>('not_started')
  ...
  useEffect(() => {
    ...
    pollingIntervalRef.current = setInterval(pollPairingStatus, 2000)
  ...
  const showTabs = !success && pairingStatus !== 'pending'

  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Connect Gateway">
      <div className="space-y-4">
        <GatewayInfo name={gatewayName} url={gatewayUrl} />

        {showTabs && (
          <ConnectionMethodTabs
            method={connectionMethod}
            onPairing={() => setConnectionMethod('pairing')}
            onToken={() => setConnectionMethod('token')}
            disabled={checking}
          />
        )}
        ...
        {success ? (
          ...
          <PendingMessage deviceId={deviceId} />
          ...
          <>
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button type="button" onClick={handleCheckPaired} disabled={checking} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50">
                {checking ? 'Checking...' : 'I Paired'}
              </Button>
            </div>
          </>
        )}
